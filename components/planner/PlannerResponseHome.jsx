...
useEffect(() => { latestScheduleRef.current = scheduleData; }, [scheduleData]);
  ...
  async function loadDetail(scheduleIdRaw) {
   const parsedId = Number(String(scheduleIdRaw ?? '').match(/^\d+$/)?.[0]);
  ...
  useEffect(() => {
    ...
    const num = Number(String(raw ?? '').match(/^\d+$/)?.[0]);
...
const extractNumericScheduleId = (obj) => {
  ...
  const getNumericScheduleId = () => {
    ...
    const fromRoute = extractNumericScheduleId(route?.params ?? {});
  ...
  const loadData = async () => {
    try {
      ...
      const parsedId = Number(String(rawId ?? '').match(/^\d+$/)?.[0]);
    ...
    const fetchDetail = async () => {
      if (scheduleId) {
        try {
          ...
          diagTransportInfo(ensurePlaceIds(detailWithId));
  ...
  try {
    ...
    if (Number.isFinite(sid)) {
      for (let i = 0; i < (editDraft.days?.length ?? 0); i++) {
        ...
        await editSchedule(dayNames, { scheduleId: sid, dayIndex: i + 1 });
      }
